...
- 콘텐츠는 `content.data.json`에서 관리되고 `src/content.jsx`로 생성됩니다.
...
6. 직접 수정 시 `content.data.json`을 편집 후 `npm run content:gen`을 실행합니다.

## 폴더 및 파일 구조
```
.
├── content.data.json    # 콘텐츠 데이터 원본 (편집기 대상)
